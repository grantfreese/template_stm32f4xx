...
#define KERNEL_VERSION                                                                                \
    (((uint32_t)tskKERNEL_VERSION_MAJOR * 10000000UL) | ((uint32_t)tskKERNEL_VERSION_MINOR * 10000UL) \
...
osThreadId_t osThreadNew(osThreadFunc_t func, void* argument, const osThreadAttr_t* attr)
{
    ...
    if (!IS_IRQ() && (func != NULL))
    {
        ...
        if (attr != NULL)
        {
            ...
            if ((attr->cb_mem != NULL) && (attr->cb_size >= sizeof(StaticTask_t)) && (attr->stack_mem != NULL)
                && (attr->stack_size > 0U))
            {
                mem = 1;
            }
    ...
    if (!IS_IRQ() && (max_count > 0U) && (initial_count <= max_count))
    {
        ...
        if (mem != -1)
        {
            ...
            else
            {
                ...
                {
                    hSemaphore =
                        xSemaphoreCreateCountingStatic(max_count, initial_count, (StaticSemaphore_t*)attr->cb_mem);
                }
                else
                {
...
osMessageQueueId_t osMessageQueueNew(uint32_t msg_count, uint32_t msg_size, const osMessageQueueAttr_t* attr)
{
    ...
    if (!IS_IRQ() && (msg_count > 0U) && (msg_size > 0U))
    {
        ...
        if (attr != NULL)
        {
            if ((attr->cb_mem != NULL) && (attr->cb_size >= sizeof(StaticQueue_t)) && (attr->mq_mem != NULL)
            ...
            {
                if ((attr->cb_mem == NULL) && (attr->cb_size == 0U) && (attr->mq_mem == NULL) && (attr->mq_size == 0U))
                {
                    mem = 0;
                }
            }
