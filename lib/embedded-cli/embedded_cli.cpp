// embedded_cli.h - CLI header
// Copyright (c) 2021 Sviatoslav Kokurin (funbiscuit) (see LICENSE.txt)
...
struct EmbeddedCliImpl
{
    ...
    const CliCommandBinding** bindings;
 ...
 * @param binding
 */
static void printBindingHelp(EmbeddedCli* cli, const CliCommandBinding* binding);
...
uint16_t embeddedCliRequiredSize(EmbeddedCliConfig* config)
{
    ...
    return (uint16_t)(CLI_UINT_SIZE
                      * (BYTES_TO_CLI_UINTS(sizeof(EmbeddedCli)) + BYTES_TO_CLI_UINTS(sizeof(EmbeddedCliImpl))
                         ...
                         + BYTES_TO_CLI_UINTS(bindingCount * sizeof(const CliCommandBinding*))
...
EmbeddedCli* embeddedCliNew(EmbeddedCliConfig* config)
{
    ...
    impl->bindings = (const CliCommandBinding**)buf;
    buf += BYTES_TO_CLI_UINTS(bindingCount * sizeof(const CliCommandBinding*));
...
bool embeddedCliAddBinding(EmbeddedCli* cli, const CliCommandBinding* binding)
{
    PREPARE_IMPL(cli);
...
void embeddedCliSetAutoComplete(EmbeddedCli* cli, bool enabled)
{
    PREPARE_IMPL(cli);
    if (enabled)
    {
...
{
    PREPARE_IMPL(cli);
    if (impl->history.itemsCount == 0 || (navigateUp && impl->history.current == impl->history.itemsCount)
...
static void initInternalBindings(EmbeddedCli* cli)
{
    static constexpr CliCommandBinding kHelpBinding = {"help", "Print list of commands", true, NULL, onHelp};
    embeddedCliAddBinding(cli, &kHelpBinding);
}
...
static void onHelp(EmbeddedCli* cli, char* tokens, void* context)
{
    ...
    if (tokenCount == 0)
    {
        // Pad name column so descriptions align on a single line per entry.
        // Width chosen to fit the longest existing command name with headroom.
        static const size_t kNameColumnWidth = 10;

        // Sort bindings alphabetically by name so `help` output is easy to scan.
        // Do this via an insertion-sorted index array so the binding table itself
        // stays in registration order (matters for lookup performance and for
        // anyone stepping through this code in a debugger). Cap the array at a
        // small fixed size; the project currently has ~20 bindings.
        static constexpr int kMaxHelpEntries = 48;
        uint16_t order[kMaxHelpEntries];
        int count = impl->bindingsCount;
        if (count > kMaxHelpEntries) count = kMaxHelpEntries;
        for (int i = 0; i < count; ++i)
        {
            // Insert impl->bindings[i] into `order` in sorted position.
            int pos = i;
            while (pos > 0 && strcmp(impl->bindings[i]->name, impl->bindings[order[pos - 1]]->name) < 0)
            {
        ...
        {
            const CliCommandBinding* b = impl->bindings[order[k]];
            writeToOutput(cli, " * ");
            writeToOutput(cli, b->name);
            size_t nameLen = strlen(b->name);
            for (size_t j = nameLen; j < kNameColumnWidth; ++j)
            {
                cli->writeChar(cli, ' ');
            }
            if (b->help != NULL)
...
static AutocompletedCommand getAutocompletedCommand(EmbeddedCli* cli, const char* prefix)
{
    ...
    for (int i = 0; i < impl->bindingsCount; ++i)
    {
        ...
        if (cmd.candidateCount == 0 || len < cmd.autocompletedLen) cmd.autocompletedLen = (uint16_t)len;
...
static void onAutocompleteRequest(EmbeddedCli* cli)
{
    ...
    // Honor the runtime autocomplete-enable flag: when off, TAB is a no-op and
    // Enter does not expand a partial command. This keeps the byte stream free
    // of save/restore-cursor escape sequences for automated CLI consumers.
    if (!IS_FLAG_SET(impl->flags, CLI_FLAG_AUTOCOMPLETE_ENABLED)) return;
...
static bool isControlChar(char c) { return c == '\r' || c == '\n' || c == '\b' || c == '\t' || c == 0x7F; }
